// Shared small components: header, footer, stars, pager, etc.
const { useState, useEffect: useEffectC } = React;
...
function SiteHeader({ route, navigate, currentUser, onLogout }) {
  const [menuOpen, setMenuOpen] = useState(false);

  useEffectC(() => { setMenuOpen(false); }, [route.name]);
  ...
  return (
    <footer className="site-footer">
      <div className="shell row">
        ...
        <div>© 2026 Recette &amp; Avis · TI616 EFREI · <a href="#/404" onClick={(e)=>{e.preventDefault();window.RA_navigate("/mentions");}}>Mentions</a></div>
      </div>
    </footer>
  );
}
...
Object.assign(window, { Stars, StarInput, CategoryPill, SiteHeader, SiteFooter, Pager });
