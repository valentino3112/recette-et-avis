...
function AdminDash({ state, navigate, currentUser }) {
  return (
    <AdminGuard currentUser={currentUser} navigate={navigate}>
      <h1 style={{fontFamily:"var(--serif)"}}>Tableau de bord</h1>
      ...
      <div className="eco-panel">
        <h3>État Green IT</h3>
        <ul>
          <li>Pages servies en HTML statique + données JSON paginées (LIMIT/OFFSET).</li>
          <li>Aucune image servie sur les listes — uniquement sur le détail, en lazy loading.</li>
          <li>Stack : Node + Express + SQLite — 0 dépendance front, ~5 dépendances back.</li>
        </ul>
      </div>
      ...
      <table className="data">
        ...
        <tbody>
          {state.comments.slice(-5).reverse().map(c => {
            ...
            const u = window.RA_userById(state.users, c.utilisateur_id);
            return (
              <tr key={c.id}>
                <td>{window.RA_shortDate(c.date_commentaire)}</td>
                <td>Commentaire</td>
...
function AdminRecipes({ state, setState, navigate, currentUser }) {
  const [page, setPage] = useStateAd(1);
  const [editing, setEditing] = useStateAd(null); // recipe object or {new:true}
  const PAGE = 8;
  ...
  function save(form) {
    if (form._new) {
      const r = { ...form, id: window.RA_uid("r"), date: new Date().toISOString().slice(0,10), auteur: currentUser.nom };
      delete r._new;
  ...
  return (
    <AdminGuard currentUser={currentUser} navigate={navigate}>
      <h1 style={{fontFamily:"var(--serif)"}}>Gestion des recettes</h1>
      ...
      <table className="data">
        <thead>
          <tr>
            <th>Titre</th><th>Catégorie</th><th>Temps</th><th>Note moy.</th><th>Avis</th><th></th>
          </tr>
        </thead>
        <tbody>
          {slice.map(r => {
...
function RecipeEditor({ initial, onSave, onCancel }) {
  ...
  return (
    <div className="profile-card mb-3" style={{maxWidth:"none"}}>
      <h2 style={{fontFamily:"var(--serif)", fontSize:18}}>{initial._new ? "Nouvelle recette" : "Modifier la recette"}</h2>
...
function AdminUsers({ state, setState, navigate, currentUser }) {
  const [page, setPage] = useStateAd(1);
  const PAGE = 10;
  ...
  return (
    <AdminGuard currentUser={currentUser} navigate={navigate}>
      <h1 style={{fontFamily:"var(--serif)"}}>Gestion des utilisateurs</h1>
          ...
          {slice.map(u => {
            ...
            return (
              <tr key={u.id}>
                ...
                <td>{window.RA_shortDate(u.date_creation)}</td>
...
function AdminComments({ state, setState, navigate, currentUser }) {
  const [page, setPage] = useStateAd(1);
  const PAGE = 10;
  const sorted = useMemoAd(() => [...state.comments].sort((a,b)=>b.date_commentaire.localeCompare(a.date_commentaire)), [state.comments]);
  ...
  return (
    <AdminGuard currentUser={currentUser} navigate={navigate}>
      <h1 style={{fontFamily:"var(--serif)"}}>Modération des commentaires</h1>
      ...
      <table className="data">
        ...
        <tbody>
          {slice.map(c => {
            ...
            return (
              <tr key={c.id}>
                <td className="muted" style={{whiteSpace:"nowrap"}}>{window.RA_shortDate(c.date_commentaire)}</td>
...
Object.assign(window, { AdminDash, AdminRecipes, AdminUsers, AdminComments });
